...
| **กลไกขากลับ in Rust** | ฟังก์ชัน `handle_post_trade_control` รีเทิร์น `Json(serde_json::json!({ ... }))` กลับไปทันทีที่ได้คำขอ | ใช้การ Broadcast ข้อความผ่าน channel (`state.tx.send`) ส่งหาไคลเอนต์ที่เชื่อมต่ออยู่ |
