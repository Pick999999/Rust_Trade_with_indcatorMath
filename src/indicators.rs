...
impl WMA {
    ...
    pub fn compute(&self, prices: &[f64]) -> IndicatorResult {
        ...
        let weights: Vec<f64> = {
            let sum: f64 = (1..=self.period).sum::<usize>() as f64;
            (1..=self.period).map(|w| w as f64 / sum).collect()
        };

        for i in self.period - 1..len {
            let wma = weighted_sum_f64(&prices[i + 1 - self.period..=i], &weights);
            result.values.push(wma);
        }
