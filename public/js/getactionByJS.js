...
// =============================================================================
function getSuggestColorV1(analysis, lossCon) {
    let suggest = "green";
    if (lossCon >= 2) {
...
function getSuggestColorV1WithReason(analysis, lossCon) {
    let suggest = "green";
    let reason;
    let conditions = [];
    let code = "V1-A";
...
// =============================================================================
function getSuggestColorV2(analysis, lossCon) {
    let suggest = "green";
    if (lossCon >= 1) {
        if (analysis.color === "red") {
            suggest = "red";
...
function getSuggestColorV2WithReason(analysis, lossCon) {
    let suggest = "green";
    let reason;
    let conditions = [];
    let code = "V2-A";
    ...
    if (lossCon >= 1) {
        ...
        reason = "loss_con < 1 -> Default (green)";
        conditions.push(`loss_con=${lossCon}`);
    }
...
// =============================================================================
function getSuggestColorV3A(analysis, lossCon) {
    let suggest = "green";
    if (lossCon >= 2) {
        let upVotes = 0;
...
function getSuggestColorV3AWithReason(analysis, lossCon) {
    let suggest = "green";
...
// =============================================================================
function getSuggestColorV3B(analysis, lossCon) {
    let suggest = "green";
    if (lossCon >= 2) {
...
function getSuggestColorV3BWithReason(analysis, lossCon) {
    let suggest = "green";
    let reason = "Default (green) loss_con < 2";
    let conditions = [];
    let code = "V3B-A";
...
// =============================================================================
function getSuggestColorV3C(analysis, lossCon) {
    let suggest = "green";
    if (lossCon >= 2) {
...
function getSuggestColorV3CWithReason(analysis, lossCon) {
    let suggest = "green";
    let reason = "Default (green) loss_con < 2";
    let conditions = [];
    let code = "V3C-A";
...
// =============================================================================
function getSuggestColorFTA(analysis, lossCon) {
...
function getSuggestColorFTAWithReason(analysis, lossCon) {
    let conditions = [
...
// =============================================================================
function getSuggestColorFTB(analysis, lossCon) {
    if (analysis.ema_short_direction !== analysis.ema_medium_direction) {
        return "idle";
    }
...
function getSuggestColorFTBWithReason(analysis, lossCon) {
    let conditions = [
        `EMA Short ${analysis.ema_short_direction}`,
        `EMA Medium ${analysis.ema_medium_direction}`,
...
// =============================================================================
// 📌 PKTrend — PK Trend Spot Evaluator Strategy
// =============================================================================
function getSuggestColorPKTrend(analysis, lossCon) {
    if (!analysis || !analysis.pkTrend) return "idle";
    const pk = analysis.pkTrend;
    const opt = { minTrendScoreStrong: 40, minCloseConvictionUp: 0.7, maxCloseConvictionDown: 0.3, allowTrapReversal: true, allowSpikeContinuation: true };
    if (typeof evaluatePkTrendEntry === 'function') {
        const res = evaluatePkTrendEntry(pk, opt);
        if (res.signal === 'BUY') return "green";
        if (res.signal === 'SELL') return "red";
        return "idle";
    }
    return "idle";
}

function getSuggestColorPKTrendWithReason(analysis, lossCon) {
    if (!analysis || !analysis.pkTrend) {
        return { suggest_color: "idle", reason: "No pkTrend data", conditions_matched: [], code: "PKT-NODATA" };
    }
    const pk = analysis.pkTrend;
    const opt = { minTrendScoreStrong: 40, minCloseConvictionUp: 0.7, maxCloseConvictionDown: 0.3, allowTrapReversal: true, allowSpikeContinuation: true };
    const conditions = [
        `Trend: ${pk.trend || 'None'}`,
        `Score: ${pk.trendScore}`,
        `CaseCode: ${pk.caseCode}`
    ];
    if (pk.closePosition !== undefined && pk.closePosition !== null) {
        conditions.push(`ClosePos: ${Number(pk.closePosition).toFixed(2)}`);
    }

    if (typeof evaluatePkTrendEntry === 'function') {
        const res = evaluatePkTrendEntry(pk, opt);
        let suggest_color = "idle";
        if (res.signal === 'BUY') suggest_color = "green";
        else if (res.signal === 'SELL') suggest_color = "red";
        return {
            suggest_color,
            reason: res.reason,
            conditions_matched: conditions,
            code: `PKT-${pk.caseCode || 'N/A'}`
        };
    }
    return { suggest_color: "idle", reason: "PkTrendEvaluator not loaded", conditions_matched: conditions, code: "PKT-NOEVAL" };
}

// =============================================================================
// 📌 Dispatcher — เลือก strategy ตาม enum
// =============================================================================
function getSuggestColorByStrategy(strategy, analysis, lossCon) {
    switch (strategy) {
        case SuggestStrategy.V1: return getSuggestColorV1(analysis, lossCon);
        case SuggestStrategy.V2: return getSuggestColorV2(analysis, lossCon);
        case SuggestStrategy.V3A: return getSuggestColorV3A(analysis, lossCon);
        case SuggestStrategy.V3B: return getSuggestColorV3B(analysis, lossCon);
        case SuggestStrategy.V3C: return getSuggestColorV3C(analysis, lossCon);
        case SuggestStrategy.FTA: return getSuggestColorFTA(analysis, lossCon);
        case SuggestStrategy.FTB: return getSuggestColorFTB(analysis, lossCon);
        case SuggestStrategy.PKTrend: return getSuggestColorPKTrend(analysis, lossCon);
        default: return getSuggestColorV1(analysis, lossCon); // Default fallback
    }
}

function getSuggestColorByStrategyWithReason(strategy, analysis, lossCon) {
    switch (strategy) {
        case SuggestStrategy.V1: return getSuggestColorV1WithReason(analysis, lossCon);
        case SuggestStrategy.V2: return getSuggestColorV2WithReason(analysis, lossCon);
        case SuggestStrategy.V3A: return getSuggestColorV3AWithReason(analysis, lossCon);
        case SuggestStrategy.V3B: return getSuggestColorV3BWithReason(analysis, lossCon);
        case SuggestStrategy.V3C: return getSuggestColorV3CWithReason(analysis, lossCon);
        case SuggestStrategy.FTA: return getSuggestColorFTAWithReason(analysis, lossCon);
        case SuggestStrategy.FTB: return getSuggestColorFTBWithReason(analysis, lossCon);
        case SuggestStrategy.PKTrend: return getSuggestColorPKTrendWithReason(analysis, lossCon);
        default: return getSuggestColorV1WithReason(analysis, lossCon); // Default fallback
    }
}
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = {
        ...
        getSuggestColorPKTrend,
        getSuggestColorPKTrendWithReason,
