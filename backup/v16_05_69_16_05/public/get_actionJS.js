...
class AnalysisObject {
    ...
    constructor(data = {}) {
        ...
        this.ema_short_val        = data.ema_short_val        || 0;   // ค่า EMA ระยะสั้น (ตัวเลข)
        this.ema_medium_val       = data.ema_medium_val       || 0;   // ค่า EMA ระยะกลาง (ตัวเลข)
        this.ema_long_val         = data.ema_long_val         || 0;   // ค่า EMA ระยะยาว (ตัวเลข)
...
// =============================================================================
// 📌 SECTION 4: getTradeAction() — ฟังก์ชันหลักสำหรับตัดสินใจเข้าเทรด
// =============================================================================
...
function getTradeAction(analysis, lossCon) {
    ...
    // ขั้นที่ 1: ตรวจสอบ ATR Spike
    // ──────────────────────────────────────────────────────────────
    // ถ้าแท่งนี้ไม่ใช่ ATR Spike → ไม่มีสัญญาณเทรด → คืนค่า WAIT ทันที
    // (ตรงตาม Rust: if !analysis.is_atr { return TradeAction::Wait; })
    if (!analysis.is_atr) {
    ...
    // ขั้นที่ 2: หา Suggest Color
    // ──────────────────────────────────────────────────────────────
    // เกิด Spike แล้ว → เรียก getSuggestColor เพื่อหาสีที่ควรเทรด
    // (ตรงตาม Rust: let suggest = get_suggest_color(&analysis.this_color, loss_con);)
    const suggest = getSuggestColor(analysis.this_color, lossCon);

    // แสดง log สำหรับ debug (ตรงตาม Rust: println!("🎯 Spike detected! ..."))
    console.log(
        `🎯 Spike detected! thisColor=${analysis.this_color}, lossCon=${lossCon}, suggest=${suggest}`
    );

    // ──────────────────────────────────────────────────────────────
    // ขั้นที่ 3: แปลง Suggest Color → Trade Action
    // ──────────────────────────────────────────────────────────────
    // (ตรงตาม Rust: match suggest.as_str() { "green" => Call, "red" => Put, _ => Wait })
    switch (suggest) {
        case "green":
            return TradeAction.CALL;   // สีเขียว → ซื้อ CALL (คาดราคาขึ้น)
        case "red":
            return TradeAction.PUT;    // สีแดง → ซื้อ PUT (คาดราคาลง)
        default:
            return TradeAction.WAIT;   // สีอื่น (edge case) → ไม่เทรด
    }
}


// =============================================================================
// 📌 SECTION 5: Export — ส่งออกทุกอย่างเพื่อให้ไฟล์อื่นเรียกใช้ได้
// =============================================================================
...
// export { TradeAction, AnalysisObject, getSuggestColor, getTradeAction };

// ── สำหรับ CommonJS (require) ──
// module.exports = { TradeAction, AnalysisObject, getSuggestColor, getTradeAction };
