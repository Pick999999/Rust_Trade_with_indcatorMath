...
pub struct AnalysisEngine {
    ...
    snap_ema_medium: f64,
    snap_ema_long: f64,
    snap_prev_ema_short: f64,
    snap_prev_ema_medium: f64,
    snap_atr: f64,
    snap_close: f64,
...
impl AnalysisEngine {
    ...
    pub fn new(config: &AppConfigPayload) -> Self {
        ...
        Self {
            ...
            snap_prev_ema_short: 0.0, snap_prev_ema_medium: 0.0,
    ...
    pub fn process_history(&mut self, ohlcv: &[OHLCV], config: &AppConfigPayload, asset_name: &str) -> Vec<AnalysisObject> {
        ...
        for i in 0..n {
            ...
            let is_ema_short_cut_type = if i > 0 {
                if ema_s[i] > ema_m[i] && ema_s[i-1] <= ema_m[i-1] { "CutUp" }
                else if ema_s[i] < ema_m[i] && ema_s[i-1] >= ema_m[i-1] { "CutDown" }
                else { "-" }
            } else { "-" };
            
            let is_ema_short_turn_type = if i > 1 {
                if ema_s[i] > ema_s[i-1] && ema_s[i-1] < ema_s[i-2] { "TurnUp" }
                else if ema_s[i] < ema_s[i-1] && ema_s[i-1] > ema_s[i-2] { "TurnDown" }
                else { "-" }
            } else { "-" };
            
            let is_ema_medium_turn_type = if i > 1 {
                if ema_m[i] > ema_m[i-1] && ema_m[i-1] < ema_m[i-2] { "TurnUp" }
                else if ema_m[i] < ema_m[i-1] && ema_m[i-1] > ema_m[i-2] { "TurnDown" }
                else { "-" }
            } else { "-" };
            ...
            results.push(AnalysisObject {
                ...
                is_ema_short_cut_type: is_ema_short_cut_type.to_string(),
                is_ema_short_turn_type: is_ema_short_turn_type.to_string(),
                is_ema_medium_turn_type: is_ema_medium_turn_type.to_string(),
        ...
        if n >= 2 {
            ...
            if n >= 3 {
                self.snap_prev_ema_short = ema_s[n - 3];
                self.snap_prev_ema_medium = ema_m[n - 3];
            } else {
                self.snap_prev_ema_short = self.snap_ema_short;
                self.snap_prev_ema_medium = self.snap_ema_medium;
            }
    ...
    pub fn update_tick(&mut self, candle: &OHLCV, is_new_candle: bool, atr_multi: f64, asset_name: &str) -> AnalysisObject {
        ...
        let is_ema_short_cut_type = if self.ema_short > self.ema_medium && self.snap_ema_short <= self.snap_ema_medium { "CutUp" }
            else if self.ema_short < self.ema_medium && self.snap_ema_short >= self.snap_ema_medium { "CutDown" }
            else { "-" };
            
        let is_ema_short_turn_type = if self.ema_short > self.snap_ema_short && self.snap_ema_short < self.snap_prev_ema_short { "TurnUp" }
            else if self.ema_short < self.snap_ema_short && self.snap_ema_short > self.snap_prev_ema_short { "TurnDown" }
            else { "-" };
            
        let is_ema_medium_turn_type = if self.ema_medium > self.snap_ema_medium && self.snap_ema_medium < self.snap_prev_ema_medium { "TurnUp" }
            else if self.ema_medium < self.snap_ema_medium && self.snap_ema_medium > self.snap_prev_ema_medium { "TurnDown" }
            else { "-" };
