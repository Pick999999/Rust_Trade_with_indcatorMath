...
async fn handle_post_generate_analysis(
    ...
    match deriv::fetch_historical_candles(&payload.asset, start_epoch, stop_epoch).await {
        Ok(candles) => {
            let config_content = fs::read_to_string("setup.json").unwrap_or_default();
            if let Ok(config) = serde_json::from_str::<AppConfigPayload>(&config_content) {
                let mut engine = analysis::AnalysisEngine::new(&config);
                let results = engine.process_history(&candles, &config, &payload.asset);
                
                // ═══ Save to analysisData folder ═══
                use chrono::Datelike;
                let now = Local::now();
...
pub struct GetAnalysisDataQuery {
    ...
    pub sdate: String, // format: DD-MM-YYYY (Thai Buddhist year เช่น 13-05-2569)
}
...
async fn handle_get_analysis_data(
    ...
    match deriv::fetch_historical_candles(&params.assetcode, start_epoch, stop_epoch).await {
        Ok(candles) => {
            if candles.is_empty() {
                return Json(serde_json::json!({
                    "status": "error",
            ...
            match serde_json::from_str::<AppConfigPayload>(&config_content) {
                Ok(config) => {
                    // ── 6. วิเคราะห์ข้อมูลด้วย AnalysisEngine ──
                    let mut engine = analysis::AnalysisEngine::new(&config);
                    let results = engine.process_history(&candles, &config, &params.assetcode);
